...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.nermes/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.nermes/.update_response`` for the
    ...
    async def send_update_prompt(
        ...
        ``~/.nermes/.update_response`` so the detached update process
        can read it.
        """
