...
def format_banner_version_label() -> str:
    ...
    base = f"Nermes Agent v{VERSION}（{RELEASE_DATE}）"
    ...
    return f"{base} · 上游 {upstream} · 本地 {local}（超前 {ahead} 个提交）"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else NERMES_MASCOT
    except Exception:
        _bskin = None
        _hero = NERMES_MASCOT
    left_lines = ["", _hero, ""]
    ...
    if os.getenv("HERMES_YOLO_MODE"):
        left_lines.append(f"[bold red]⚠ YOLO 模式[/] [dim {dim}]— 已绕过所有批准提示[/]")
    ...
    if skills_by_category:
        for category in sorted(skills_by_category.keys()):
            ...
            if len(skill_names) > 8:
                ...
                skills_str = ", ".join(display_names) + f" +{len(skill_names) - 8} 个"
            else:
    ...
    summary_parts.append("/help 查看命令帮助")
