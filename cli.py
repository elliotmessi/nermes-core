#!/usr/bin/env python3
"""
Nermes Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Nermes Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills nermes-agent-dev,github-auth
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.nermes/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.nermes/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)

    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If NERMES_IGNORE_USER_CONFIG=1 is set (via ``nermes chat --ignore-user-config``),
    the user config at ``~/.nermes/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({NERMES_HOME}/config.yaml)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.nermes/logs/.
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        print("\033[31m✗ --worktree 需要在 git 仓库中运行。\033[0m")
        print("  请先 cd 到您的项目仓库，然后运行 hermes -w")
        return None
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"\033[31m✗ 创建工作树失败: {result.stderr.strip()}\033[0m")
            return None
    except Exception as e:
        print(f"\033[31m✗ 创建工作树失败: {e}\033[0m")
        return None
    ...
    print(f"\033[32m✓ 工作树已创建:\033[0m {wt_path}")
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    ...
    if has_unpushed:
        print(f"\n\033[33m⚠ 工作树有未推送的提交，已保留: {wt_path}\033[0m")
        print(f"  如需手动清理: git worktree remove --force {wt_path}")
    ...
    _active_worktree = None
    print(f"\033[32m✓ 工作树已清理: {wt_path}\033[0m")
...
# ASCII Art - Nermes Mascot (compact, fits in left panel)
NERMES_MASCOT = """[#CD7F32]         ╭──────╮         [/]
[#CD7F32]        ╱        ╲        [/]
[#CD7F32]       │  ◉    ◉  │       [/]
[#FFBF00]       │     ▔     │       [/]
[#FFBF00]       │  ╰────╯  │       [/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Nermes Agent") if _skin else "Nermes Agent"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Nermes v{_version} ({_release_date})"
    else:
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.nermes/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.nermes/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for the Nermes Agent.
    ...
    def __init__(
        ...
        # checkpoint repos under ~/.nermes/checkpoints/.  Opt-in via
    ...
    def _open_external_editor(self, buffer=None) -> bool:
        ...
        if self._command_running:
            _cprint(f"{_DIM}请等待当前命令完成后，再打开编辑器。{_RST}")
            return False
        if self._sudo_state or self._secret_state or self._approval_state or getattr(self, "_slash_confirm_state", None) or self._clarify_state:
            _cprint(f"{_DIM}请先完成当前提示后，再打开编辑器。{_RST}")
            return False
        target_buffer = buffer or getattr(app, "current_buffer", None)
        if target_buffer is None:
            _cprint(f"{_DIM}当前没有可用的活动输入缓冲区用于外部编辑器。{_RST}")
            return False
        try:
            ...
            _cprint(f"{_DIM}无法打开外部编辑器: {exc}{_RST}")
            return False
            ...
            if isinstance(_primary_exc, AuthError):
                ...
                for _fb in _fb_chain:
                    ...
                    try:
                        ...
                        _cprint(f"⚠️  主要认证失败 — 切换到备用方案: {_fb_provider} / {_fb_model}")
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                _cprint(f"\033[1;31m未找到会话: {self.session_id}{_RST}")
                _cprint(f"{_DIM}请使用之前 CLI 运行的会话 ID (hermes sessions list)。{_RST}")
                return False
            ...
            if resolved_id and resolved_id != self.session_id:
                ChatConsole().print(
                    f"[{_DIM}]会话 {_escape(self.session_id)} 已被压缩为 "
                    f"{_escape(resolved_id)}；正在恢复包含对话记录的后继会话。[/]"
                )
            ...
            if restored:
                ...
                ChatConsole().print(
                    f"[bold {_accent_hex()}]↻ 已恢复会话[/] "
                    f"[bold]{_escape(self.session_id)}[/]"
                    f"[bold {_accent_hex()}]{_escape(title_part)}[/] "
                    f"({msg_count} 条用户消息, {len(restored)} 条总消息)"
                )
            else:
                ChatConsole().print(
                    f"[bold {_accent_hex()}]会话 {_escape(self.session_id)} "
                    f"已找到但无消息。从头开始。[/]"
                )
        ...
        try:
            ...
            if self._pending_title and self._session_db and self.agent:
                try:
                    ...
                    _cprint(f"  无法应用待处理标题: {e}")
                    # Keep _pending_title so it can be retried after row creation succeeds
            return True
        except Exception as e:
            ChatConsole().print(f"[bold red]无法初始化代理: {e}[/]")
            return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            self._console_print()
            self._console_print(
                f"[yellow]⚠️  上下文长度仅为 {ctx_len:,} 个 token — "
                f"这对带工具的代理使用来说很可能太低了。[/]"
            )
            self._console_print(
                "[dim]   Nermes 需要至少 16k–32k。工具架构 + 系统提示本身就需要约 4k–8k。[/]"
            )
        ...
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Hermes 3 & 4 模型不具备代理能力，"
                "不适合用于 Nermes Agent。[/]"
            )
            self._console_print(
                "[dim]   它们缺乏代理工作流所需的工具调用能力。"
                "请考虑使用具有代理能力的模型 (Claude, GPT, Gemini, DeepSeek 等)。[/]"
            )
            self._console_print(
                "[dim]   切换命令: /model sonnet  或  /model gpt5[/]"
            )
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if resolved_id and resolved_id != self.session_id:
            self._console_print(
                f"[dim]会话 {self.session_id} 已被压缩为 "
                f"{resolved_id}；正在恢复包含对话记录的后继会话。[/]"
            )
            self.session_id = resolved_id
            resolved_meta = self._session_db.get_session(self.session_id)
            if resolved_meta:
        ...
        if restored:
            ...
            accent_color = _accent_hex()
            self._console_print(
                f"[{accent_color}]会话 {self.session_id} 已找到但无"
                f"消息。从头开始。[/]"
            )
            return False
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.nermes/images/ and appends the path to
                ...
                if not stat and not diff:
                    ...
                    if diff:
                        ...
                        if len(diff_lines) > 80:
                            print("\n".join(diff_lines[:80]))
                            print(f"\n  ... ({len(diff_lines) - 80} 行更多内容，显示前 80 行)")
                        else:
                            print(f"\n{diff}")
            else:
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in {"list", "ls"}:
            ...
            if len(parts) < 3:
                print("  用法：/snapshot restore <快照-id>")
    ...
    def _handle_agents_command(self):
        ...
        _cprint(f"  代理: {'运行中' if agent_running else '空闲'}")
    ...
    def _handle_paste_command(self):
        ...
        if has_clipboard_image():
            if self._try_attach_clipboard_image():
                ...
                _cprint(f"  {_DIM}(>_<) 剪贴板包含图片但提取失败{_RST}")
        else:
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        if arg:
            ...
            while idx >= 0 and not _assistant_copy_text(assistant[idx].get("content")):
                idx -= 1
            if idx < 0:
                _cprint("  Assistant 回复中暂无可复制内容。")
                return

        text = _assistant_copy_text(assistant[idx].get("content"))
        if not text:
            _cprint("  该 Assistant 回复中无可复制内容。")
            return
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            _cprint(f"  {_DIM}现在输入你的问题（或使用 --image 单次查询模式）：{_remainder}{_RST}")
        ...
        for img_path in images:
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    if announce:
                        _cprint(f"  {_DIM}⚠ 视觉分析失败 — 已包含路径以便重试{_RST}")
            except Exception as e:
                enriched_parts.append(
                ...
                if announce:
                    _cprint(f"  {_DIM}⚠ 视觉分析出错 — 已包含路径以便重试{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   运行 'nermes setup' 进行配置[/]")
        except Exception:
    ...
    def show_toolsets(self):
        ...
        print("  (*) = 当前已启用")
        print()
        print("  提示：使用 'all' 或 '*' 启用全部工具集")
        print("  示例：nermes --toolsets web,terminal")
        print()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        print()
        print("  使用 /resume <会话ID或标题> 从中断处继续。")
        print()
        return True
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            _cprint("  将当前会话移交给该平台的主频道。")
            _cprint("  CLI 会话在此结束；稍后可用 /resume 恢复。")
            return True
        ...
        if getattr(self, "_agent_running", False):
            _cprint("  Agent 正忙。请等待当前轮次结束后重试 /handoff。")
            return True
        ...
        while _time.time() < deadline:
            ...
            if current == "failed":
                ...
                _cprint(f"  交接失败：{err}")
                _cprint("  你的 CLI 会话完好。可重试 /handoff，或在该平台手动 /resume。")
                return True
            _time.sleep(0.5)
        ...
        try:
            ...
            pass
        _cprint("  等待网关超时。`nermes gateway` 是否在运行？")
        _cprint("  你的 CLI 会话完好。")
        return True
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.nermes/sessions/saved/.
    ...
    def _handle_gquota_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            self._console_print(f"  [red]配额查询失败：[/] {exc}")
            return
                ...
                if qcmd.get("type") == "exec":
                    ...
                    if exec_cmd:
                        try:
                            ...
                            self._console_print(f"[bold red]快捷命令错误：{e}[/]")
                    else:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Nermes is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Nermes is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Nermes is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Nermes Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Nermes Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nermes Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Nermes Agent",
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.nermes/skills/ and queue a note for the
        next user turn.
    ...
    def run(self):
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Nermes Agent")
...
def main(
    ...
    """
    Nermes Agent CLI - Interactive AI Assistant
