...
def _print_fast_version_info() -> None:
    ...
    print(f"Nermes v{__version__} ({__release_date__})")
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'nermes')
    """
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "看起来 Nermes 尚未配置 -- 未找到 API 密钥或提供商。"
        )
        print()
        print("  运行：nermes setup")
        print()
    ...
    # user's ~/.nermes/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("您将如何使用 WhatsApp 与 Nermes？")
        print()
        print("  1. 独立机器人号码（推荐）")
...
def select_provider_and_model(args=None):
    ...
    # a leftover OPENAI_BASE_URL in ~/.nermes/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nermes/.env if the active provider is not 'custom'.
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.nermes/.env by hand. The
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.nermes/.env by hand.
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Nermes URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic 风格 API 端点的模型。Nermes 将探测您的")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Nermes will install it now (the preflight below "
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Nermes turns to `copilot --acp`.")
    print("  Nermes currently starts its own ACP subprocess for each request.")
    print("  Nermes uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.nermes/.env`` by hand.
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Nermes typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Nermes, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Nermes will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _update_via_zip(args):
    """Update Nermes Agent by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Nermes behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Nermes couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Nermes couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Nermes behaves unexpectedly.")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Hermes repository (upstream).")
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Nermes Desktop, exit any open `hermes` REPLs, and")
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Nermes Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Nermes Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Nermes Desktop, exit other `hermes` REPLs, stop the "
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.nermes/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.nermes/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.nermes/logs/update.log`` and to silently absorb
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Nermes Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _cmd_update_pip(args):
    """Update Nermes via pip (for PyPI installs)."""
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.nermes)")
            else:
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Nermes {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Nermes {mf.hermes_requires}")
...
def cmd_logs(args):
    """View and filter Nermes log files."""
...
def main():
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.nermes/.env.  Currently supports "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Nermes.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Nermes")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Nermes CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Nermes with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Nermes Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.nermes/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.nermes/scripts/. Pass empty string to clear. "
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.nermes/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.nermes/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Nermes Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Nermes setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nermes Agent. Use 'hermes debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        ...
        description="Create a zip archive of your entire Nermes configuration, "
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.nermes/checkpoints/",
    ...
    import_parser = subparsers.add_parser(
        ...
        description="Extract a previously created Nermes backup into your "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Nermes Agent configuration",
    )
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            "Manage MCP server connections and run Nermes as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hermes mcp add' to connect to a new server, or\n"
            "'hermes mcp serve' to expose Nermes conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Nermes as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Nermes",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Nermes",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.nermes/ (by default a "
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Nermes Agent as an ACP (Agent Client Protocol) server",
        description="Start Nermes Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Nermes ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    def cmd_acp(args):
        """Launch Nermes Agent as an ACP server."""
        try:
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Nermes profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nermes Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Nermes log files",
