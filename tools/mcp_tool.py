...
Configuration is read from ~/.nermes/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nermes/logs/mcp-stderr.log), tagged with the
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Nermes is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.nermes/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.nermes/.env`` loaded at startup).
    """
    try:
